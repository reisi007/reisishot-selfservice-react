...
import {
  CHART_SETTINGS, ChartVisibilities, SetChartVisibilityType, Totals,
} from '../../../charts/helper';
...
export function RealityCheck(yearData: YearDataType & StatisticChartProps) {
  ...
  const maxYear = useMemo(() => Math.max(...Object.keys(rawData)
    .map((e) => parseInt(e, 10)))
    .toString(10), [rawData]);
  const data = useMemo(() => Object.keys(CHART_SETTINGS)
    .filter((shootingType) => visibilities[shootingType] ?? true)
    .map((shootingType) => ({
      key: shootingType,
      value: rawData[maxYear][shootingType] ?? 0,
...
function useRenderMaxYearPieChart(size: number, visibilities: ChartVisibilities, data: Array<{ key: string; value: number }>, totals: Totals) {
  ...
  const filteredData = useMemo(() => data.filter(({ key: shootingType }) => visibilities[shootingType] ?? true), [data, visibilities]);
  return (
    <Pie
      data={filteredData}
...
function useRenderExpectationPieChart(size: number, visibilities: ChartVisibilities) {
  const data = useMemo(() => Object.entries(CHART_SETTINGS)
    .filter(([shootingType]) => visibilities[shootingType] ?? true)
    .map(([key, { expectedPercentage: value }]) => ({
      key,
      value,
    })), [visibilities]);
  ...
  const totals: Totals = useMemo(
    () => {
      const total = data.map((e) => e.value)
        .reduce((a, b) => a + b, 0);
