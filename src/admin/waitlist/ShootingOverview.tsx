...
function ShootingType({item, loginData}: { item: WaitlistItemWithRegistrations, loginData: LoginData }) {
  ...
  const [modal, setVisibility] = useModal(setModalOpen => <StatisticModalContent itemId={itemId} loginData={loginData}
...
function StatisticModalContent({
  ...
  const onSubmit = useCallback(({
    ...
    const isMinor = !is18;
    store({itemId, isGroup, isMinor}, loginData)
      .then(() => {
        setSubmitting(false);
        setModalOpen(false);
      });
  ...
  return <div>
    ...
    <Formik<StatisticsModalForm> initialValues={{is18: false, isGroup: false}}
                                 onSubmit={onSubmit}>
      {formik =>
        <>
          ...
          <SubmitButton formik={formik} requestInfo={request} allowInitialSubmit={true}/>
        </>
      }
    </Formik>
  </div>;
