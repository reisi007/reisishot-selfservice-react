...
function Modal({children, isVisible, setVisible}: ModalProps) {
  const classes = classNames({
    'hidden': !isVisible,
  });

  return <div onClick={() => setVisible(false)}
...
export function useModal(children: (setVisible: Dispatch<SetStateAction<boolean>>) => ReactNode, initialVisibility: boolean = false) {
  ...
  const modal = <Modal isVisible={isVisible} setVisible={setVisible} children={children}/>;
